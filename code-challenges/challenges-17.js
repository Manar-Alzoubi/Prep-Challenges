...
const recursionPattern = (int1, int2) => {
    ...
    const arr1=[];
    let count=0;
    const mirror=(int1,int2) =>{
       if(int1>0){
         arr.push(int1);
         arr1.push(int1);
         count=count+1;
         mirror(int1-int2 , int2);
       }
         else{
         arr.push(int1);   
...
const filterLinks = (str) => {
    let str2= ( str.match(/\w[www]?\w+\.+\w\w{2,3}/gm)).toString();
    let result = str2.replace(",", "");
    return result;
}
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    ...
    while(start <= finish)
    {
        let letterAtStart = str[start];
        let letterAtEnd = str[finish];
        if (!(letterAtStart >= 'a' && letterAtStart <= 'z'))
            start++;
        else if(!(letterAtEnd >= 'a' && letterAtEnd <= 'z'))
            finish--;
        else if( letterAtStart == letterAtEnd)
        {
            start++;
            finish--;
        }
